...
export default function StoriesPage() {
  ...
  async function handleDelete(story: { id: string; title: string }) {
  ...
  return (
    <div className="-m-8 min-h-[calc(100vh-0px)] bg-admin-bg">
                    ...
                    <div className="flex items-center justify-between gap-3 border-b border-admin-border px-4 py-3 last:border-0">
                      <div className="min-w-0 flex items-center gap-2 flex-wrap">
                        ...
                        <a
                          ...
                          title="Open the story on the site"
                        >
                          /stories/{story.slug}
                          <ExternalLink className="h-3 w-3" />
                        </a>
                        {!story.isPublished && <Pill>Draft</Pill>}
                        ...
                        <RowMenu
                          items={[
                            ...
                            { label: "History…", icon: <History className="h-3.5 w-3.5" />, onSelect: () => setHistoryFor(story) },
                            {
                              ...
                              onSelect: () => togglePublish(story),
                            },
                            "divider",
                            { label: "Delete…", icon: <Trash2 className="h-3.5 w-3.5" />, danger: true, onSelect: () => handleDelete(story) },
                          ]}
                        />
                      </div>
                    </div>
      ...
      <Drawer
        ...
        footer={
          <>
            <Button type="submit" form="new-story-form" kind="primary">
              Create &amp; edit
            </Button>
      ...
      >
        <form id="new-story-form" onSubmit={handleCreate} className="space-y-4">
          <Field label="Title" htmlFor="ns-title">
            <Input id="ns-title" name="title" placeholder="Story title" required />
          </Field>
        </form>
      </Drawer>

      <Drawer
        open={!!(editSettings && editingStory)}
        ...
        footer={
          <>
            <Button type="submit" form="story-settings-form" kind="primary">
              Save settings
            </Button>
            <Button type="button" kind="ghost" onClick={() => setEditSettings(null)}>
              Cancel
            </Button>
          </>
        }
